...
-- Template filenames map 1:1 to a Storage object key; enforce uniqueness so
-- two type rows can never share (and delete) the same template.
create unique index if not exists idx_jenislabel_namafail
  on public.tblJenisLabel(namaFail);
create unique index if not exists idx_jenisworksheet_namafail
  on public.tblJenisWorksheet(namaFail);
